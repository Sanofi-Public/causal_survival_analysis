...
all_estimates <- function(data, sample.size, tau,
  ...
  available_estimators <- c(
    ...
    "G_formula (S-learner)","AIPCW", "AIPTW-AIPCW", "SurvRM2 - KM", 
  ...
  # RMST estimate with AIPCW transformation
  if ("AIPCW" %in% estimator) {
    for (survival_method in nuisance_survival) {
      for (censoring_method in nuisance_censoring) {
        ATE_aipcw <- AIPCW(data, tau = tau, 
                     X.names.censoring = X.names.censoring, 
                     X.names.outcome = X.names.outcome,
                     nuisance_censoring = censoring_method, 
                     nuisance_Qt = survival_method, 
                     n.folds = n.folds_censoring)$RMST
        if (survival_method == "survival forest"){survival_name = "Forest"}
        else{survival_name = "Cox"}
        if (censoring_method == "survival forest"){
          censoring_name = "Forest"}
        else{censoring_name = "Cox"}
        if (censoring_name == survival_name){
          est_name <- paste("AIPCW (", survival_name, ")", sep = "")
        }
        else{
          est_name <- paste("AIPCW (", survival_name," & ", 
                            censoring_name , ")", sep = "")}
        results <- rbind(results, data.frame(
...
load("results/simulation_rct1_final_AIPCW.RData")

simulation_rct1 <- rbind(simulation_rct1,simulation_rct1_aipcw)
```
...
simulation_rct1 <- simulation_rct1 %>%
  ...
  filter(estimator !="AIPCW (Cox)")%>%
  filter(estimator !=  "IPCW KM (Forest)")%>%
  filter(estimator !=  "IPCW (Forest)")%>%
  filter(estimator !="BJ (Forest)")%>%
  filter(estimator !="AIPCW (Forest)")%>%
  filter(estimator !="AIPCW (Cox & Forest)")%>%
  filter(estimator !="AIPCW (Forest & Cox)")
```
...
simulation_rct1 <- simulation_rct1 %>%
  ...
  filter(estimator != "RISCA - G_formula (S-learner)") %>%
  filter(estimator != "RISCA - IPTW KM (Forest)")%>%
  filter(estimator != "AIPTW-AIPCW (Forest & Cox)") %>%
  filter(estimator != "AIPCW (Forest & Cox)") %>%
  filter(estimator != "AIPTW-AIPCW (Cox & Forest)")%>%
  filter(estimator != "AIPCW (Cox & Forest)")
```
...
ggsave(
  ...
  device = "pdf",                          # format
  width = 23, height = 8,                  # dimensions en pouces
...
load("results/simulation_rct2_final_AIPCW.RData")
simulation_rct2 <- rbind(simulation_rct2,simulation_rct2_aipcw)
```
...
ggsave(
  ...
  plot = simulation_graph_rct2,            # objet graphique
  device = "pdf",                          # format
  width = 13, height = 8,                  # dimensions en pouces
  units = "in"                             # unités (ici, pouces)
)
```
...
load("results/simulation_obs1_final_AIPCW.RData")
simulation_obs1 <- rbind(simulation_obs1,simulation_obs1_aipcw)
...
load("results/simulation_obs2_final_AIPCW.RData")
simulation_obs2 <- rbind(simulation_obs2,simulation_obs2_aipcw)
...
```{r, eval = params$plug_in_bias}
data <- simulate_data_obs(n=50000, tau=25,   coefC = 0.03, 
  parsC = c(0.7, 0.3, -0.25, -0.1), scenario = "Obs2")
test_plug_in <- g_formula_T_learner(data, tau=25, X.names.outcome = c("X1", "X2", "X3", "X4"), nuisance_survival = "survival forest", n.folds=5)
bias <- test_plug_in - truth_tau4
```
...
tau <- 1
estimators <-  c(
  "Naive", "IPTW-IPCW KM", "IPTW-BJ", "G_formula (T-learners)", "AIPTW-AIPCW" 
)
